...
export function RelatedTools({ links, heading = "Related calculators & guides" }: { links: RelatedLink[]; heading?: string }) {
  ...
  return (
    <section className="mt-12 rounded-3xl border bg-card p-6 shadow-[var(--shadow-soft)]" style={{ background: "var(--gradient-card)" }} aria-labelledby="related-tools-heading">
      <h2 id="related-tools-heading" className="text-lg font-semibold tracking-tight">{heading}</h2>
      <p className="mt-1 text-sm text-muted-foreground">Explore more free tools on Utilikit — every calculator is fast, ad-light and works right in your browser.</p>
